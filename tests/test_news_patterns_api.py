...
from packages.shared.models import Asset, NewsEvent, NewsImpact, Pattern, PatternPerformance
...
def test_list_patterns_filters_by_asset(client, db_session):
    ...
    resp = client.get(f"/api/patterns?asset_id={asset_a.id}&limit=50")
    assert resp.status_code == 200
...
def test_pattern_performance_endpoint(client, db_session):
    ...
    resp = client.get("/api/patterns/performance")
    assert resp.status_code == 200
