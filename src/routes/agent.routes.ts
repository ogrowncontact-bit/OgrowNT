...
agentRouter.get("/agent", async (_req, res) => {
...
agentRouter.put("/agent", requireRole(...WRITE_ROLES), async (req, res) => {
  const business = currentBusiness(res);
...
agentRouter.get("/knowledge", async (req, res) => {
  const business = currentBusiness(res);
...
agentRouter.post("/knowledge", requireRole(...WRITE_ROLES), async (req, res) => {
  const business = currentBusiness(res);
...
agentRouter.patch("/knowledge/:id", requireRole(...WRITE_ROLES), async (req, res) => {
  const business = currentBusiness(res);
...
agentRouter.delete("/knowledge/:id", requireRole(...WRITE_ROLES), async (req, res) => {
  const business = currentBusiness(res);
  const existing = await prisma.knowledgeEntry.findFirst({ where: { id: req.params.id, businessId: business.id } });
  if (!existing) {
    res.sendStatus(404);
...
agentRouter.get("/rules", async (_req, res) => {
  const business = currentBusiness(res);
...
agentRouter.post("/rules", requireRole(...WRITE_ROLES), async (req, res) => {
  const business = currentBusiness(res);
...
agentRouter.patch("/rules/:id", requireRole(...WRITE_ROLES), async (req, res) => {
  const business = currentBusiness(res);
...
agentRouter.delete("/rules/:id", requireRole(...WRITE_ROLES), async (req, res) => {
  const business = currentBusiness(res);
  const existing = await prisma.businessRule.findFirst({ where: { id: req.params.id, businessId: business.id } });
  if (!existing) {
    res.sendStatus(404);
