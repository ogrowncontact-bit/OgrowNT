 ...
 */
async function seedAssessment(prisma: PrismaClient, assessment: AssessmentConfig) {
...
export interface SeedDatabaseOptions {
  /** Overrides process.env.ADMIN_PASSWORD — used by callers that generate a password at seed time instead of reading it from an env var. */
  adminPassword?: string;
  /** Overrides process.env.ADMIN_EMAIL. */
  adminEmail?: string;
}

export interface SeedDatabaseResult {
  adminEmail: string;
  /** True if a password was passed in or read from ADMIN_PASSWORD; false means the dev-only fallback was used. */
  usedExplicitPassword: boolean;
}

/**
 * Populates a fresh database with the assessment catalog, dimension pool,
 * recommendation graph, default AI personas, and the seed admin user.
 * Safe to call only against a database that doesn't already have version 1
 * of these assessments — see seedAssessment's docstring.
 */
export async function seedDatabase(prisma: PrismaClient, options: SeedDatabaseOptions = {}): Promise<SeedDatabaseResult> {
  ...
  for (const assessment of allAssessments) {
    await seedAssessment(prisma, assessment);
  }
  ...
  const adminEmail = (options.adminEmail ?? process.env.ADMIN_EMAIL ?? "admin@inner.dev").toLowerCase();
  const explicitPassword = options.adminPassword ?? process.env.ADMIN_PASSWORD;
  const adminPassword = explicitPassword ?? "changeme-dev-only";
  ...
  console.log(
    `Seeded admin user "${adminEmail}"${explicitPassword ? "" : " with the dev-only default password — set ADMIN_EMAIL/ADMIN_PASSWORD env vars before seeding a real environment"}.`
  );

  return { adminEmail, usedExplicitPassword: !!explicitPassword };
}
